...
class TestMaksukortti(unittest.TestCase):
	...
	def test_negatiivisen_summan_lataaminen_ei_muuta_kortin_saldo(self):
		self.kortti.lataa_rahaa(-5)

		self.assertEqual(str(self.kortti), "Kortilla on rahaa 10 euroa")
	def test_kortilla_pystyy_ostaa_edullisen_luonaan_kun_rahaa_on_lounaan_verran(self):
		kortti = Maksukortti(2.5)
		kortti.syo_edullisesti()

		self.assertEqual(str(kortti), "Kortilla on rahaa 0.0 euroa")
	def test_kortilla_pystyy_ostaa_maukkaan_luonaan_kun_rahaa_on_lounaan_verran(self):
